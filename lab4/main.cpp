...
static void createRightPanel(HWND wnd, HINSTANCE inst) {
    ...
    const int buttonsH = BTN_H * 9 + 16 + 20 + 4 * 6 + 12 * 3;
    ...
    g_hBtnLoadPlugin = makeBtn(wnd, inst, "Load Plugin...", IDC_BTN_LOAD_PLUGIN, px + 12, y, 12);
...
static void repositionControls(HWND wnd) {
    ...
    deferBtn(hdwp, g_hBtnLoadPlugin, px + 12, y, 12);
    ...
    case WM_COMMAND: {
        ...
        if (cmd == IDC_BTN_EDIT || cmd == IDM_EDIT_SELECTED) {
            ...
            if (sel == LB_ERR || static_cast<size_t>(sel) >= g_list.count()) {
                MessageBoxA(wnd, "Select an object in the list.", "Lab4",
            ...
            if (!codec) {
                MessageBoxA(wnd, "No editor for this type.", "Lab4",
                    MB_OK | MB_ICONERROR);
                return 0;
            }
        ...
        if (cmd == IDC_BTN_SAVE || cmd == IDM_SAVE_BINARY) {
            ...
            if (pickSavePath(wnd, path, static_cast<int>(sizeof(path)))) {
                if (ShapeBinaryRegistry::saveListToFile(g_list, path))
                    MessageBoxA(wnd, "List saved.", "Lab4", MB_OK | MB_ICONINFORMATION);
                else
                    MessageBoxA(wnd, "Save failed.", "Lab4", MB_OK | MB_ICONERROR);
            }
            return 0;
        }
        ...
        if (cmd == IDC_BTN_LOAD || cmd == IDM_LOAD_BINARY) {
            ...
            if (pickOpenPath(wnd, path, static_cast<int>(sizeof(path)))) {
                if (ShapeBinaryRegistry::loadListFromFile(g_list, path)) {
                    ...
                    redraw();
                    refreshListBox();
                    MessageBoxA(wnd, "List loaded.", "Lab4", MB_OK | MB_ICONINFORMATION);
                }
                else {
                    MessageBoxA(wnd, "Load failed.", "Lab4", MB_OK | MB_ICONERROR);
                }
            }
            return 0;
        }

        if (cmd == IDC_BTN_LOAD_PLUGIN) {
            ...
            if (pickPluginPath(wnd, path, static_cast<int>(MAX_PATH))) {
                ...
                if (PluginLoader::loadPluginAtPath(full, wnd)) {
                    refreshCombo();
                    MessageBoxA(wnd, "Plugin loaded successfully.", "Lab4",
                        MB_OK | MB_ICONINFORMATION);
                }
                else {
                    MessageBoxA(wnd, "Plugin load failed (bad signature or incompatible).",
                        "Lab4", MB_OK | MB_ICONERROR);
                }
            }
            return 0;
        }
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE, LPSTR, int show) {
    ...
    const char* cls = "SimpleShapeEditorLab4";
    WNDCLASSA wc = {};
    ...
    HWND w = CreateWindowExA(0, cls, "Graphical Editor — Lab4 (plugins + binary I/O)",
