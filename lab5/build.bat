@echo off
echo Building lab5 host (ShapesLab.exe) with MSVC...
...
cl /nologo /EHsc /Fe:ShapesLab.exe main.cpp ShapeList.cpp ShapeRenderer.cpp ShapeFactory.cpp Solid3D.cpp ^
   ShapeBinaryRegistry.cpp ShapeBinaryCodecs.cpp StorageProcessorRegistry.cpp Lab3Dialogs.cpp HostServices.cpp PluginLoader.cpp PluginTrust.cpp ^
   ...
   lab1.res user32.lib gdi32.lib comdlg32.lib shell32.lib bcrypt.lib
...
call "%~dp0plugins\PluginStar\build_plugin.bat"
if %ERRORLEVEL% neq 0 (
    echo PluginStar build failed.
    pause
    exit /b 1
)
call "%~dp0plugins\PluginArchiveRLE\build_plugin.bat"
if %ERRORLEVEL% neq 0 (
    echo PluginArchiveRle build failed.
    pause
    exit /b 1
)
call "%~dp0plugins\PluginArchiveZero\build_plugin.bat"
if %ERRORLEVEL% neq 0 (
    echo PluginArchiveZero build failed.
    pause
    exit /b 1
)
call "%~dp0plugins\PluginBadgeA\build_plugin.bat"
if %ERRORLEVEL% neq 0 (
    echo PluginBadgeA build failed.
    pause
    exit /b 1
)
call "%~dp0plugins\PluginBadgeB\build_plugin.bat"
if %ERRORLEVEL% neq 0 (
    echo PluginBadgeB build failed.
    pause
    exit /b 1
)
echo All builds OK. Copy ShapesLab.exe next to plugins\ folder to run.
pause
