...
constexpr UINT IDM_PLUGIN_LOAD = 1030;
constexpr UINT IDM_PLUGIN_SETTINGS = 1031;
...
constexpr UINT IDC_BTN_LOAD_PLUGIN = 3013;
constexpr UINT IDC_BTN_PLUGIN_CFG = 3014; 
...
static HWND g_hBtnLoadPlugin = nullptr;
static HWND g_hBtnPluginCfg = nullptr;
...
static void refreshListBox() {
    ...
    for (size_t i = 0; i < g_list.count(); ++i) {
        ...
        std::string lab = std::to_string(i + 1) + ". " +
            ShapeBinaryRegistry::listLabelFor(s);
        SendMessageA(g_hListBox, LB_ADDSTRING, 0,
            reinterpret_cast<LPARAM>(lab.c_str()));
    }
}
...
static void updatePluginCfgLabel() {
    if (!g_hBtnPluginCfg) return;
    if (StorageProcessorRegistry::hasProcessors()) {
        auto ids = StorageProcessorRegistry::enabledProcessorIds();
        std::string label = "Processors (" + std::to_string(ids.size()) + ")";
        SetWindowTextA(g_hBtnPluginCfg, label.c_str());
    }
    else {
        SetWindowTextA(g_hBtnPluginCfg, "No Processors");
    }
}

static bool pickSavePath(HWND owner, char* buf, int n) {
    OPENFILENAMEA ofn = {};
...
static bool pickOpenPath(HWND owner, char* buf, int n) {
    OPENFILENAMEA ofn = {};
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = owner;
    ofn.lpstrFile = buf;
...
static bool pickPluginPath(HWND owner, wchar_t* buf, int n) {
    OPENFILENAMEW ofn = {};
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = owner;
    ofn.lpstrFile = buf;
...
static INT_PTR CALLBACK ParamsDlgProc(HWND dlg, UINT msg, WPARAM wp, LPARAM) {
    if (msg == WM_INITDIALOG) {
        ...
        else if (g_currentTool == "star")      hint = "star (plugin): cx cy halfWidth halfHeight";
        else if (g_currentTool == "badge_a" ||
            g_currentTool == "badge_b")   hint = "badge plugin: left top right bottom";
        SetWindowTextA(dlg, hint);
        return TRUE;
    }
    if (msg == WM_COMMAND && LOWORD(wp) == IDOK) {
        char line[1024] = { 0 };
        GetDlgItemTextA(dlg, IDC_PARAMS_EDIT, line,
        ...
        if (parseInts(line, params)) {
            if (g_currentTool == "polygon" && params.size() == 1) {
                ...
                if (n >= 3 && n <= 200) {
                    MouseToolRegistry::setPointCountOverride("polygon",
...
static void showParamsDialog(HWND owner) {
    DialogBoxParamA(GetModuleHandle(nullptr),
        MAKEINTRESOURCEA(IDD_PARAMS), owner, ParamsDlgProc, 0);
}
...
static void setupMenu(HWND wnd) {
    ...
    HMENU plugins = CreateMenu();
    AppendMenuA(plugins, MF_STRING, IDM_PLUGIN_LOAD, "Load plugin DLL...");
    AppendMenuA(plugins, MF_STRING, IDM_PLUGIN_SETTINGS, "Plugin settings...");
    AppendMenuA(bar, MF_POPUP, reinterpret_cast<UINT_PTR>(plugins), "&Plugins");

    SetMenu(wnd, bar);
}

static HWND makeBtn(HWND wnd, HINSTANCE inst,
    const char* label, UINT id,
    int x, int& y, int gap = 4) {
    HWND h = CreateWindowExA(0, "BUTTON", label,
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        x, y, BTN_W, BTN_H,
        wnd, (HMENU)(UINT_PTR)(id), inst, nullptr);
    y += BTN_H + gap;
    return h;
}

static HWND makeLabel(HWND wnd, HINSTANCE inst,
    const char* text,
    int x, int& y,
    int w, int h = 18, int gap = 22) {
    HWND hw = CreateWindowExA(0, "STATIC", text,
        WS_CHILD | WS_VISIBLE,
        x, y, w, h,
        wnd, nullptr, inst, nullptr);
    y += gap;
    return hw;
}

static void createToolbar(HWND wnd, HINSTANCE inst) {
    g_hLabelShape = CreateWindowExA(0, "STATIC", "Shape:",
        WS_CHILD | WS_VISIBLE | SS_LEFT,
        8, 12, 48, 18,
        wnd, nullptr, inst, nullptr);

    g_hCombo = CreateWindowExA(0, "COMBOBOX", nullptr,
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | CBS_DROPDOWNLIST,
        58, 8, COMBO_W, COMBO_DH,
        wnd, (HMENU)(UINT_PTR)(IDC_TOOLBAR_COMBO), inst, nullptr);
    refreshCombo();

    CreateWindowExA(0, "BUTTON", "Color",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        186, 8, 64, BTN_H,
        wnd, (HMENU)(UINT_PTR)(IDC_BTN_COLOR), inst, nullptr);

    CreateWindowExA(0, "BUTTON", "Add (Dialog)",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        256, 8, 96, BTN_H,
        wnd, (HMENU)(UINT_PTR)(IDC_BTN_ADD_DIALOG), inst, nullptr);

    RECT rc; GetClientRect(wnd, &rc);
    g_hSepHorz = CreateWindowExA(0, "STATIC", nullptr,
        WS_CHILD | WS_VISIBLE | SS_ETCHEDHORZ,
        0, TOOLBAR_H - 1, rc.right, 2,
        wnd, nullptr, inst, nullptr);
}
...
static void createRightPanel(HWND wnd, HINSTANCE inst) {
    ...
    int y = TOOLBAR_H + 8;
    g_hSepVert = CreateWindowExA(0, "STATIC", nullptr,
        WS_CHILD | WS_VISIBLE | SS_ETCHEDVERT,
        px, TOOLBAR_H, 2, H - TOOLBAR_H,
        wnd, nullptr, inst, nullptr);
    g_hLabelShapes = makeLabel(wnd, inst, "Shapes:", px + 8, y, PANEL_W - 16);
    const int buttonsH = BTN_H * 10 + 16 + 20 + 4 * 7 + 12 * 3;
    const int listH = H - y - buttonsH - 8;
    const int safeH = listH > 20 ? listH : 20;
    g_hListBox = CreateWindowExA(WS_EX_CLIENTEDGE, "LISTBOX", nullptr,
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY,
        px + 6, y, PANEL_W - 14, safeH,
        wnd, (HMENU)(UINT_PTR)(IDC_SHAPE_LISTBOX), inst, nullptr);
    y += safeH + 8;
    ...
    g_hBtnLoadPlugin = makeBtn(wnd, inst, "Load Plugin...", IDC_BTN_LOAD_PLUGIN, px + 12
    g_hBtnPluginCfg = makeBtn(wnd, inst, "No Processors", IDC_BTN_PLUGIN_CFG, px + 12, y, 12);
...
static void deferCtrl(HDWP& hdwp, HWND h, int x, int y, int w, int ht) {
    if (h) hdwp = DeferWindowPos(hdwp, h, nullptr, x, y, w, ht,
...
static void deferLabel(HDWP& hdwp, HWND h, int x, int& y,
    int w, int ht = 18, int gap = 22) {
...
static void repositionControls(HWND wnd) {
    ...
    const int buttonsH = BTN_H * 10 + 16 + 20 + 4 * 7 + 12 * 3;
    const int safeH = (H - y - buttonsH - 8) > 20 ? (H - y - buttonsH - 8) : 20;
    ...
    deferBtn(hdwp, g_hBtnLoadPlugin, px + 12, y);
    deferBtn(hdwp, g_hBtnPluginCfg, px + 12, y, 12);
...
static LRESULT CALLBACK WndProc(HWND wnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_COMMAND: {
        ...
        if (cmd == IDC_TOOLBAR_COMBO && notif == CBN_SELCHANGE) {
            ...
            if (sel != CB_ERR) {
                SendMessageA(g_hCombo, CB_GETLBTEXT, sel,
        ...
        if (cmd == IDC_BTN_ADD_DIALOG || cmd == IDM_ADD_DIALOG) {
        ...
        if (cmd == IDC_BTN_EDIT || cmd == IDM_EDIT_SELECTED) {
            ...
            if (sel == LB_ERR || static_cast<size_t>(sel) >= g_list.count()) {
                MessageBoxA(wnd, "Select an object in the list.", "Lab5",
            ...
            if (!codec) {
                MessageBoxA(wnd, "No editor for this type.", "Lab5",
                    MB_OK | MB_ICONERROR);
                return 0;
            }
        ...
        if (cmd == IDC_BTN_SAVE || cmd == IDM_SAVE_BINARY) {
            ...
            if (pickSavePath(wnd, path, static_cast<int>(sizeof(path)))) {
                if (ShapeBinaryRegistry::saveListToFile(g_list, path))
                    MessageBoxA(wnd, "List saved.", "Lab5", MB_OK | MB_ICONINFORMATION);
                else
                    MessageBoxA(wnd, "Save failed.", "Lab5", MB_OK | MB_ICONERROR);
            }
            return 0;
        }
        ...
        if (cmd == IDC_BTN_LOAD || cmd == IDM_LOAD_BINARY) {
            ...
            if (pickOpenPath(wnd, path, static_cast<int>(sizeof(path)))) {
                if (ShapeBinaryRegistry::loadListFromFile(g_list, path)) {
                    ...
                    redraw();
                    refreshListBox();
                    MessageBoxA(wnd, "List loaded.", "Lab5", MB_OK | MB_ICONINFORMATION);
                }
                else {
                    MessageBoxA(wnd, "Load failed (wrong file or unsupported data).",
                        "Lab5", MB_OK | MB_ICONERROR);
                }
            }
            return 0;
        }
        if (cmd == IDC_BTN_LOAD_PLUGIN || cmd == IDM_PLUGIN_LOAD) {
            wchar_t path[MAX_PATH] = { 0 };
            if (pickPluginPath(wnd, path, static_cast<int>(MAX_PATH))) {
                wchar_t full[MAX_PATH] = {};
                GetFullPathNameW(path, MAX_PATH, full, nullptr);
                if (PluginLoader::loadPluginAtPath(full, wnd)) {
                    refreshCombo();
                    updatePluginCfgLabel();
                    MessageBoxA(wnd, "Plugin loaded successfully.", "Lab5",
                        MB_OK | MB_ICONINFORMATION);
                }
                else {
                    MessageBoxA(wnd, "Plugin load failed (bad signature or incompatible).",
                        "Lab5", MB_OK | MB_ICONERROR);
                }
            }
            return 0;
        }
        if (cmd == IDC_BTN_PLUGIN_CFG || cmd == IDM_PLUGIN_SETTINGS) {
            StorageProcessorRegistry::showSettingsDialog(wnd);
            updatePluginCfgLabel();
            return 0;
        }
        if (cmd == IDC_BTN_INPUT_MOUSE || cmd == IDM_INPUT_MOUSE) {
            g_useMouse = true;  syncInputButtons(); return 0;
        }
        if (cmd == IDC_BTN_INPUT_DLG || cmd == IDM_INPUT_DIALOG) {
...
static void loadPluginsFromCommandLine() {
    int argc = 0;
    LPWSTR* argv = CommandLineToArgvW(GetCommandLineW(), &argc);
    if (!argv) return;
    for (int i = 1; i < argc; ++i) {
        const wchar_t* arg = argv[i];
        const size_t n = wcslen(arg);
        if (n < 5 || _wcsicmp(arg + n - 4, L".dll") != 0) continue;
        wchar_t full[MAX_PATH] = {};
        if (GetFullPathNameW(arg, MAX_PATH, full, nullptr) > 0)
            PluginLoader::loadPluginAtPath(full, nullptr);
    }
    LocalFree(argv);
}

int WINAPI WinMain(HINSTANCE inst, HINSTANCE, LPSTR, int show) {
    loadPluginsFromCommandLine();
    PluginLoader::loadPluginsFromDefaultFolder(nullptr);

    const char* cls = "SimpleShapeEditorLab5";
    WNDCLASSA wc = {};
    ...
    HWND w = CreateWindowExA(0, cls,
        "Graphical Editor — Lab5 (plugins + archiving)",
